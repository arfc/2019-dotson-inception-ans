...
It is infeasible to get enough real measurements to properly train a neural network. 
Thus all of the data used to train the neural network here has been simulated using GADRAS-DRF (Mitchell and Harding, 2014). 
...
In the fully connected layer, dropout regularization is used to force the neural network to learn newpathways. 
Another method to prevent overfitting is the use of hyperparameters to optimize performance. 
There is no way to know which hyperparameters will influence the model before training, so a random hyperparameter search is performed to identify which are important (Bergstra and Bengio, 2012). 
...
Once reusing data from previous work (Kamuda, et. al,  2018) and again with the improved datasets. 
...
We hypothesize that the INN would exhibit an increase in robustness commensurate to its computationalcomplexity and that training the neural networks with larger and varied datasets will also improve its robustness. 
Improvements in the ability to identify isotopes present in a sample of radioactive material will have implications for [$\emph{add something here...}$]
